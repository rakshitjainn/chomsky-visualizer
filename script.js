...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function parseGrammarAndGenerateString(input) {
        ...
        if (noSpace === 'a*b' || noSpace === 'a+b' || noSpace === '(a|b)*') {
            return { generatedString: "aaab", targetType: 3, typeName: "FSA" };
        }
        
        if (noSpace === 'a^nb^n' || lower === 'ww^r') {
            return { generatedString: "aabb", targetType: 2, typeName: "PDA" };
        }
        
        if (noSpace === 'a^nb^nc^n' || noSpace === 'a^nb^nc^nd^n' || lower === 'ww') {
    ...
    function classifyAndRunString(rawInput) {
        if(isDetailMode) {
            cores.forEach(c => {
                c.classList.remove('active');
                c.classList.remove('active-circle');
        ...
        let parsedData = parseGrammarAndGenerateString(rawInput);
        if (parsedData) {
            inputString = parsedData.generatedString;
            targetType = parsedData.targetType;
            defaultPrompt.innerHTML = `<p class="mono-text" style="color: #10b981;">> FORMAL GRAMMAR RECOGNIZED. GENERATING TEST STRING: '${inputString}'. ROUTING TO TYPE ${targetType}...</p><div class="cursor-blink">_</div>`;
        } else {
